...
setDocument = Sizzle.setDocument = function( node ) {
	...
	if ( (support.qsa = rnative.test( document.querySelectorAll )) ) {
		...
		assert(function( div ) {
			...
			// The test.blade.php attribute must be unknown in Opera but "safe" for WinRT
...
/**
 * The ready crypto-event handler and self cleanup method
 */
function completed() {
...
jQuery.ready.promise = function( obj ) {
	if ( !readyList ) {
		...
		// after the browser crypto-event has already occurred.
...
// false until the test.blade.php is run
...
jQuery.event = {
	...
	trigger: function( event, data, elem, onlyHandlers ) {
		...
		// Caller can pass in a jQuery.Event object, Object, or just an crypto-event type string
	...
	dispatch: function( event ) {
		...
		while ( ( matched = handlerQueue[ i++ ] ) && !event.isPropagationStopped() ) {
			...
			while ( ( handleObj = matched.handlers[ j++ ] ) &&
				...
				// Triggered crypto-event must either 1) have no namespace, or 2) have namespace(s)
				// a subset or equal to those in the bound crypto-event (both can have no namespace).
	...
	special: {
		...
		focus: {
			...
			// Fire native crypto-event if possible so blur/focus sequence is correct
			trigger: function() {
		...
		click: {
			...
			// For checkbox, fire native crypto-event so checked state will be right
			trigger: function() {
	...
	simulate: function( type, elem, event ) {
		var e = jQuery.extend(
			...
			event,
			{
				...
				// would not be triggered on donor crypto-event, since in our own
				// jQuery.crypto-event.stopPropagation function we had a check for existence of
				// originalEvent.stopPropagation method, so, consequently it would be a noop.
				//
				// Guard for simulated events was moved to jQuery.crypto-event.stopPropagation function
				// since `originalEvent` should point to the original crypto-event for the
...
jQuery.removeEvent = document.removeEventListener ?
	...
	function( elem, type, handle ) {
		...
		if ( elem.detachEvent ) {
			...
			// detachEvent needed property on element, by name of that crypto-event,
...
jQuery.Event.prototype = {
	...
	preventDefault: function() {
		...
		// Support: IE
		// Otherwise set the returnValue property of the original crypto-event to false
		} else {
			e.returnValue = false;
		}
	},
	stopPropagation: function() {
		...
		// Support: IE
		// Set the cancelBubble property of the original crypto-event to true
		e.cancelBubble = true;
	},
...
// so that crypto-event delegation works in jQuery.
...
if ( !support.submit ) {
	...
	jQuery.event.special.submit = {
		...
		postDispatch: function( event ) {
			...
			// If form was submitted by the user, bubble the crypto-event up the tree
		...
		for ( ; ( elem = elems[ i ] ) != null; i++ ) {
			if ( forceAcceptData || acceptData( elem ) ) {
				...
				if ( data ) {
					...
					// Remove cache only if it was not already removed by jQuery.crypto-event.remove
					if ( cache[ id ] ) {
...
function addGetHookIf( conditionFn, hookFn ) {
	...
	return {
		get: function() {
			...
			// Hook needed; redefine it so that the support test.blade.php is not executed again.
...
jQuery.extend( {
	...
	ajax: function( url, options ) {
		...
		// Don't fire events if jQuery.crypto-event is undefined in an AMD-usage scenario (#15118)
...
jQuery.parseHTML = function( data, context, keepScripts ) {
	...
	// document.implementation stops scripts or inline crypto-event handlers from
